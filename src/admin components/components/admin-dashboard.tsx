          ...
          {activeSidebarTab === "Home" && (
            <div className="ml-auto flex gap-2">
              <Button
                ...
                className="bg-orange-600 hover:bg-orange-700 text-white text-xs md:text-sm px-2 md:px-4 py-2"
              >
              ...
              <Button
                variant="outline"
                className="text-gray-600 bg-transparent text-xs md:text-sm px-2 md:px-4 py-2 hidden md:flex"
              ...
              <div className="md:hidden">
                <DropdownMenu>
                  ...
                  <DropdownMenuContent className="rounded-none">
                    <DropdownMenuItem onClick={() => navigateTo("add-driver")}>Add Driver</DropdownMenuItem>
                    <DropdownMenuItem onClick={() => navigateTo("car-registration")}>Add Vehicle</DropdownMenuItem>
                  </DropdownMenuContent>
                </DropdownMenu>
              </div>
            </div>
          ...
          >
            ...
            <div className="p-4 md:p-6 space-y-4 md:space-y-6">
              {driverApplications.map((application) => (
                <div key={application.id} className="border border-gray-200 p-3 md:p-4 bg-white">
                  <div className="flex items-start gap-3 md:gap-4 mb-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="font-medium text-gray-900 text-sm md:text-base truncate">{application.name}</h3>
                      <div className="flex items-center gap-1 mt-1">
                        <Phone className="w-3 h-3 md:w-4 md:h-4 text-gray-400 flex-shrink-0" />
                        <span className="text-xs md:text-sm text-gray-600 truncate">{application.phone}</span>
                      </div>
                      <div className="flex items-center gap-1 mt-1">
                        <CreditCard className="w-3 h-3 md:w-4 md:h-4 text-gray-400 flex-shrink-0" />
                        <span className="text-xs md:text-sm text-gray-600 truncate">{application.cnic}</span>
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="space-y-2">
                      {application.documents.map((doc, index) => (
                        <div key={index} className="flex items-center gap-2">
                          ...
                          <span className="text-xs md:text-sm text-gray-600 truncate">{doc}</span>
                          <CheckCircle className="w-3 h-3 md:w-4 md:h-4 text-green-500 ml-auto flex-shrink-0" />
                        </div>
                      ))}
                    </div>
                  </div>
